    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <NavLink to='/' onClick={()=>setOpen(false)}>
                    ...
                    <img className="h-14" src={assets.QuickBasket} alt="logo" />
            </NavLink>
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {!user ?(  <button onClick ={()=>setShowUserLogin(true)}className="cursor-pointer px-8 py-2 bg-green-500 hover:bg-green-600 transition text-white rounded-full">
                    ...
                    <ul className="hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-40 rounded-md text-sm z-40">
                        <li onClick={()=>navigate("my-orders")}className="p-1.5 pl-3 hover:bg-green-500/10 cursor-pointer">☰ My Orders</li>
                        <li onClick={logout} className="p-1.5 pl-3 hover:bg-green-500/10 cursor-pointer">⟶ Logout</li>
                    </ul>
                </div>)}
            </div>
            ...
            {open &&
                ...
                {!user ? (<button onClick={()=>{setOpen(false) ; setShowUserLogin(true)}}className="cursor-pointer px-6 py-2 mt-2 bg-green-500 hover:bg-primary transition text-white rounded-full text-sm">
                    Login
                </button>):(<button onClick={logout}className="cursor-pointer px-6 py-2 mt-2 bg-green-500 hover:bg-primary transition text-white rounded-full text-sm">
                    Logout
                </button>)}
            </div>}
