    ...
    return product && (
        <div className="mt-12">
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                                ...
                                <img key={i}src={i<4 ?assets.star_icon: assets.star_dull_icon} alt="rating" className='w-3.5 md:w-4'/>
            ...
            <div className='flex flex-col items-center mt-20'>
                ...
                <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 mt-6 w-full gap-3 md:gap-6'>
                    {relatedProducts.filter((product)=>product.inStock).map((product , index)=>(
                        <ProductCard key={index} product={product}/>
                    ))}
                </div>
                <button onClick={()=>{navigate("/products"); scroll(0,0)}} className='mx-auto cursor-pointer px-12 my-16 py-2.5 border rounded text-green-500 hover:bg-green-500/10 transition'>See More</button>
            </div>
        </div>
    );
};
